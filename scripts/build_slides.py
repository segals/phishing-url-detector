...
!! IMPORTANT !!  The committed presentation/url_detector.pptx was edited by hand after
being generated by this script (the presented version is the authoritative one).
Re-running this script will OVERWRITE those manual edits. Only run it if you intend to
regenerate the deck from scratch.
"""
import os
import pandas as pd
...
def _box(s, left, top, width, height):
...
def header(s, title, kicker=None, tag=None):
    ...
    if kicker:
        _run(tf.add_paragraph(), kicker, 14, ACCENT, italic=True)
    if tag:
        tb = _box(s, 10.7, 0.42, 2.5, 0.5)
        tb.paragraphs[0].alignment = 2
...
def picture(s, name, left=7.0, top=1.95, width=6.0):
    p = f"{FIG}/{name}.png"
    if os.path.exists(p):
        s.shapes.add_picture(p, Inches(left), Inches(top), width=Inches(width))
...
def bottom_note(s, text, color=RED):
    tf = _box(s, 0.7, 6.55, 12, 0.7)
    _run(tf.paragraphs[0], text, 17, color, bold=True)
...
p = tf.add_paragraph()
_run(p, "…and why “99% accuracy” in URL phishing detection is mostly an illusion",
     20, RGBColor(0x9a, 0xd1, 0xc9), italic=True)
p = tf.add_paragraph()
...
# =====================================================================
s = slide()
header(s, "The question, and the project")
...
# =====================================================================
s = slide()
header(s, "The data", "real, public, recent")
...
# =====================================================================
s = slide()
header(s, "How we chose the features", "feature engineering")
...
# =====================================================================
s = slide()
header(s, "The 40 features — six families")
...
# =====================================================================
s = slide()
header(s, "Two features worth the math", "information theory · edit distance")
tf = _box(s, 0.7, 1.9, 12, 4.4)
_run(tf.paragraphs[0], "Shannon entropy of the host:   H = − Σ p · log₂(p)", 22, INK, bold=True)
p = tf.add_paragraph()
p.space_after = Pt(6)
_run(p, "“google” = repeating, predictable letters → LOW entropy;  "
        "“x7qz9wvt” ≈ random → HIGH entropy.", 16, GREY)
p = tf.add_paragraph()
_run(p, "→ catches algorithmically-generated (DGA) phishing domains.", 16, ACCENT, italic=True)
p = tf.add_paragraph()
p.space_before = Pt(20)
_run(p, "Brand look-alike:   Levenshtein edit distance to a known brand", 22, INK, bold=True)
p = tf.add_paragraph()
p.space_after = Pt(6)
_run(p, "edit distance = min single-character inserts / deletes / substitutions "
        "to turn one word into another.", 16, GREY)
p = tf.add_paragraph()
_run(p, "“paypa1” is edit-distance 1 from “paypal”  → flag distance 1–2 as a typosquat.",
        16, ACCENT, italic=True)
...
# =====================================================================
s = slide()
header(s, "Step 1 — train it. It looks perfect: F1 ≈ 1.0")
f1_all = val("T0_artifact_decomposition", "representation", "provided", "f1")
f1_one = val("T0_artifact_decomposition", "representation", "URLSimilarityIndex", "f1")
f1_lex = val("T0_artifact_decomposition", "representation", "honest", "f1")
bullets(s, [f"All provided features → F1 {f1_all}",
            f"ONE feature (URLSimilarityIndex) alone → F1 {f1_one}  → basically the label (leakage)",
            f"My honest hand-built lexical features → F1 {f1_lex}",
...
# =====================================================================
s = slide()
header(s, "How we evaluate honestly", "the metric toolkit — and why")
...
# =====================================================================
s = slide()
header(s, "Discovery 2 — SHAP confirms it: is_https drives everything", tag="DISCOVERY 2")
bullets(s, ["SHAP = a faithful way to credit each feature for the prediction (game theory)",
            "#1 driver = is_https, ~3× the next feature",
...
# =====================================================================
s = slide()
rec_https = val("T3_arms_race", "attack", "https_upgrade", "recall_attacked")
rec_mimicry = val("T3_arms_race", "attack", "homepage_mimicry", "recall_attacked")
header(s, "Discovery 3 — attack it: robust to tricks, but not to the artifact", tag="DISCOVERY 3")
bullets(s, ["Homoglyph / typosquat (char tricks) → recall stays ~0.99",
            "  a STRUCTURAL detector ignores letters → resists what BREAKS a text model (nice contrast!)",
            f"Just switch HTTP → HTTPS → recall {rec_https}",
            f"Host on a clean HTTPS domain (mimicry) → recall {rec_mimicry}  (total evasion)",
...
# =====================================================================
s = slide()
auc_iso = val("T10_anomaly", "unsupervised_detector", "IsolationForest", "auc")
auc_lof = val("T10_anomaly", "unsupervised_detector", "LocalOutlierFactor", "auc")
header(s, "Cross-check — can we catch it with NO labels?", "anomaly detection")
bullets(s, ["Train only on LEGITIMATE URLs; flag anything unusual as phishing",
            f"Isolation Forest → AUC {auc_iso}   (isolates odd points with random splits)",
            f"Local Outlier Factor → AUC {auc_lof}   "
...
# =====================================================================
s = slide()
f1_id = val("T4_cross_dataset", "setting", "in-distribution", "f1")
f1_ood = val("T4_cross_dataset", "setting", "full model", "f1")
auc_id = val("T4_cross_dataset", "setting", "in-distribution", "auc")
auc_ood = val("T4_cross_dataset", "setting", "full model", "auc")
rec_live = val("T4_cross_dataset", "setting", "OpenPhish", "recall_phishing")
header(s, "Discovery 4 — it does NOT generalise", tag="DISCOVERY 4")
bullets(s, [f"Train PhiUSIIL, test an INDEPENDENT 651k dataset: F1 {f1_id} → {f1_ood}",
            f"AUC {auc_id} → {auc_ood}  — below 0.5 = ‘confidently backwards’",
            ...
            f"BUT live OpenPhish phishing: recall {rec_live} "
            "(today’s phishing is still messy → caught)"], width=6.5)
...
# =====================================================================
s = slide()
header(s, "Why it inverts — distance metrics pinpoint it", "KS & Wasserstein")
bullets(s, ["Measure how far each feature’s distribution SHIFTS between the two datasets",
            ...
            "  …which are EXACTLY the top SHAP drivers → it built on the least "
            "transferable signals"], width=6.5)
...
# =====================================================================
s = slide()
header(s, "Conclusion — a useful second opinion, not a solution")
...
# =====================================================================
s = slide()
header(s, "Course concepts used in this project")
bullets(s, ["EDA · distributions · skewness/kurtosis · robust stats · Pearson vs Spearman · Mann–Whitney U",
            ...
            "Domain shift / concept drift · distance metrics (KS, Wasserstein) · "
            "feature ablation · error analysis"],
        size=15.5)
...
p = tf.add_paragraph()
_run(p, "github.com/segals/phishing-url-detector   ·   reproducible: python run_all.py",
     16, RGBColor(0x9a, 0xd1, 0xc9))
p = tf.add_paragraph()
